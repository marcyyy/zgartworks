<?php
  include 'includes/portfolio_update.inc.php';
  ...
  if (isset($_GET['edit']))
  {
    $id = $_GET['edit'];
    $edit_state = true;
    $rec = mysqli_query($conn, "SELECT * FROM portfolio WHERE id=$id");
    $record = mysqli_fetch_array($rec);

    $style = $record['pf_style'];
    $title = $record['pf_title'];
    $image = $record['pf_image'];
    $id = $record['id'];
  }
    ...
    <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
      <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">Portfolio</h1>
            <a href="portfolio.php" class="btn btn-sm btn-outline-secondary">Back</a>
      </div>
      ...
      if (isset($_GET["upload"])) {
        if ($_GET["upload"] == "success") {
          ...
          Image uploaded successfully!<br>Check the change/s on the <a href='gallery.php' style='text-decoration:none' target='_blank'>website.</a>
          </div>";
        }
        else if ($_GET["upload"] == "update") {
          echo "<div class='alert alert-success alert-dismissible'>
          <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
          Image updated successfully!<br>Check the change/s on the <a href='gallery.php' style='text-decoration:none' target='_blank'>website.</a>
          </div>";
        }
        else if ($_GET["upload"] == "delete") {
          echo "<div class='alert alert-success alert-dismissible'>
          <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
          Image deleted successfully!<br>Check the change/s on the <a href='gallery.php' style='text-decoration:none' target='_blank'>website.</a>
          </div>";
        }
      }
      ?>

      <form method="post" action="includes/portfolio_update.inc.php" enctype="multipart/form-data">
        <input type="hidden" name="id" value="<?php echo $id; ?>">

        <div class="col-md-12 mb-3">
          <?php if ($edit_state == true):
        echo "<img class='imgcenter' src='includes/portfolio/".$image."' height='400px' onerror='this.src='assets/icons/logo.jpg';'/>";
        endif?>
        </div>
             ...
             <div class="col-md-5 mb-3">
              ...
              <select  class="form-control" id="style" name="style" required>
                   <?php while ($row = mysqli_fetch_array($result)) { 
                      if(($row['id'])===$style) {  ?>
                        <option value="<?php echo $row['id']; ?>" selected><?php echo $row['style_name']; ?></option>
                      <?php } else { ?>
        ...
        <table class="table table-striped table-sm">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td><?php echo $row['pf_title']; ?></td>
              <td><a href="portfolio_update.php?edit=<?php echo $row['id']; ?>">Edit</a></td>
              <td><a href="includes/portfolio_update.inc.php?del=<?php echo $row['id']; ?>">Delete</a></td>
            </tr>
            <?php } ?>
          </tbody>
        </table>
