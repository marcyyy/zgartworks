<?php
	...
	$results = mysqli_query($conn, "SELECT * FROM orders_merch WHERE status = 'Pending' ") or die( mysqli_error($conn));
	...
	if (isset($_GET['accept']))
	  {
		...
		header('location: orders.php?acceptsuccess='.$id.'');
	  }
	...
	if (isset($_GET['decline']))
	  {
		...
		mysqli_query($conn, "UPDATE orders_merch SET status = '$status' WHERE id=$id") or die( mysqli_error($conn));
		header('location: orders.php?declinesuccess='.$id.'');
	  }
?>
    ...
    <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
      <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        ...
        <div class="btn-group mr-2">
            <a href="merch_history.php" class="btn btn-sm btn-outline-secondary">Processed Orders</a>
          </div>
      </div>
      ...
      <?php
	      	if (isset($_GET["acceptsuccess"])) 
		          ...
		          You have accepted Merch Order #". $id ."!<br>
		          Proceed to <a style='text-decoration:none' href='merch_history.php'>Processed Orders</a>
		          </div>";
			  	}
			elseif (isset($_GET["declinesuccess"])) 
		          ...
		          echo "<div class='alert alert-success alert-dismissible'>
		          <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
		          You have declined Merch Order #". $id ."!<br>
		          Proceed to <a style='text-decoration:none' href='merch_history.php'>Processed Orders</a>
		          </div>";
			}
	    ?>
        ...
        <table class="table table-striped table-sm">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td><a href="orders.php?details=<?php echo $row['id']; ?>">Items</a></td>

              <td><a onclick="location.href='mailto:<?php echo $row['email']; ?>?subject=Zgartworks Merch Order Confirmation&body=Hi <?php echo $row['name']; ?>! Your Merch Order has been accepted. Reply to this email so we can process your order and payment. Have a good day!';" href="orders.php?accept=<?php echo $row['id']; ?>" target="_self">Accept</a></td>

              <td><a onclick="location.href='mailto:<?php echo $row['email']; ?>?subject=Zgartworks Merch Order Confirmation&body=Hi <?php echo $row['name']; ?>! We regret to inform you that your Merch Order has been declined. This can be due to an item being out of stock. You can reply to this email to further discuss arrangements or if you have any inquiries! Have a good day.';" href="orders.php?decline=<?php echo $row['id']; ?>" target="_self">Decline</a></td>
